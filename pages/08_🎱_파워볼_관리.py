...
from utils.powerball_economy import (
    ...
    repay_adena_from_saved_reward_lines,
    reward_run_exists,
)
...
if "pb_live_interval" not in st.session_state:
    st.session_state.pb_live_interval = 3
...
st.sidebar.caption(
    "DB는 **autocommit**으로 읽어, 게임 서버가 넣은 최신 배팅·결과가 보이게 했습니다. "
    "자동 갱신은 **streamlit-autorefresh**·Streamlit 버전에 따라 다릅니다. "
    "숫자가 안 바뀌면 「DB 새로고침」 또는 `pip install streamlit-autorefresh` 후 GM 재실행."
)
...
def _pb_run_live_refresh_hooks() -> str:
    ...
    try:
        ...
        sec = max(3, int(round(_pb_refresh_seconds())))
        ms = sec * 1000
        # 주기 변경 시 컴포넌트가 다시 붙도록 key에 초 단위 포함 (고정 key면 간격만 바꿔도 타이머가 꼬일 수 있음)
        st_autorefresh(interval=ms, limit=None, key=f"pb_gm_autorefresh_{sec}s")
        return "autorefresh"
    except ImportError:
        pass
    except Exception:
...
def _pb_update_config_line(text: str, key: str, rhs: str) -> str:
    """`KEY = value` 한 줄을 치환하거나 없으면 POWERBALL_REWARD_CLASS_DEFAULTS 앞에 삽입."""
    line = f"{key} = {rhs}"
    pat = rf"(?m)^\s*{re.escape(key)}\s*=.*$"
    if re.search(pat, text):
        return re.sub(pat, line, text)
    ins_pat = r"(?m)^POWERBALL_REWARD_CLASS_DEFAULTS\s*="
    m_ins = re.search(ins_pat, text)
    if m_ins:
        return text[: m_ins.start()] + line + "\n\n" + text[m_ins.start() :]
    return text.rstrip() + "\n" + line + "\n"


def _update_gm_config_powerball_reward_pool(
    pct_0_100: float,
    w_knight: int,
    w_wizard: int,
    w_elf: int,
    ...
    """일일 포상: 순이익 대비 풀 % 및 직업 가중치를 config.py에 기록."""
    cfg_path = Path(__file__).resolve().parents[1] / "config.py"
    try:
        text = cfg_path.read_text(encoding="utf-8", errors="ignore")
    except Exception as e:
    ...
    p = max(0.0, min(100.0, float(pct_0_100))) / 100.0
    p = round(p, 6)
    wk = max(0, int(w_knight))
    ww = max(0, int(w_wizard))
    we = max(0, int(w_elf))
    wd = max(0, int(w_darkelf))
    wr = max(0, int(w_royal))
    if wk + ww + we + wd + wr <= 0:
        return False, "가중치를 하나 이상 1 이상으로 두세요."

    p_str = f"{p:.10f}".rstrip("0").rstrip(".") or "0"
    text2 = text
    text2 = _pb_update_config_line(text2, "POWERBALL_REWARD_POOL_PERCENT_OF_PROFIT", p_str)
    text2 = _pb_update_config_line(text2, "POWERBALL_REWARD_WEIGHT_KNIGHT", str(wk))
    text2 = _pb_update_config_line(text2, "POWERBALL_REWARD_WEIGHT_WIZARD", str(ww))
    text2 = _pb_update_config_line(text2, "POWERBALL_REWARD_WEIGHT_ELF", str(we))
    text2 = _pb_update_config_line(text2, "POWERBALL_REWARD_WEIGHT_DARKELF", str(wd))
    text2 = _pb_update_config_line(text2, "POWERBALL_REWARD_WEIGHT_ROYAL", str(wr))

    try:
        cfg_path.write_text(text2, encoding="utf-8")
        return True, "config.py 반영 완료 (GM 툴 재시작 또는 페이지 새로고침 후 반영)"
    except Exception as e:
        return False, f"config.py 저장 실패: {e}"
...
def _pb_render_tab1_metrics() -> None:
    ...
    # 구버전 연결 호환: 묵시적 트랜잭션 잔재 제거 (db_manager에서도 처리)
    try:
        conn = db.connection
        if conn is not None and not conn.get_autocommit():
            conn.commit()
    except Exception:
        pass
...
def _pb_render_tab3_live_panel() -> None:
    ...
    if sm2 and sm2.server_profit > 0:
        ...
        pools = pws.get("pools") or pws.get("pools_by_prefix") or {}
        pct = float(pws.get("pool_percent", 0))
        total_p = int(pws.get("total_reward_pool", 0))
        st.caption(
            f"순이익 **{sm2.server_profit:,}** × 포상 **{pct:.2%}** → 총 풀 **{total_p:,}** 아데나. "
            "체크한 직업만 **config 가중치** 비율로 분배: "
            f"기사 **{int(pools.get('knight', 0)):,}** · 법사 **{int(pools.get('wizard', 0)):,}** · 요정 **{int(pools.get('elf', 0)):,}** · "
            f"다크 **{int(pools.get('darkelf', 0)):,}** · 군주 **{int(pools.get('royal', 0)):,}** "
            "(네 직업은 직업당 **12:7:3**, 군주는 **7:3:2**로 1~3위에 지급.)"
        )
    elif sm2:
    ...
    if already:
        st.warning(
            "이 날짜는 이미 `powerball_reward_run`에 기록되어 있습니다. **전체 정산**은 다시 실행할 수 없습니다. "
            "서버가 순이익 0으로 **자동 원장만** 남긴 경우는, 지급 버튼을 누르면 그 원장을 지우고 다시 시도합니다."
        )
        _reward_lines = db.fetch_all(
            "SELECT id FROM powerball_reward_line WHERE reward_date = %s",
            (d2.isoformat(),),
        ) or []
        if _reward_lines:
            st.caption(
                "원장·`powerball_reward_line` 은 있는데 인벤에 아데나가 안 들어갔다면, 아래 **아데나만 재지급**을 사용하세요 "
                "(이미 지급된 뒤 또 누르면 **중복 지급**됩니다)."
            )
            if st.button(
                "💰 원장(powerball_reward_line) 기준으로 아데나만 재지급",
                key="pb_repay_adena_only",
                type="secondary",
            ):
                ok_r, msg_r = repay_adena_from_saved_reward_lines(db, d2)
                if ok_r:
                    queue_feedback("success", msg_r)
                else:
                    queue_feedback("error", msg_r)
                st.rerun()
...
with st.expander("📌 설계·제한 사항 (요청하신 규칙과 구현 해석)", expanded=False):
    ...
    st.markdown(
        ...
        **일일 포상 (포상 탭·체크박스·`config.py`)**  
        - 순이익 대비 총 포상 풀 **`POWERBALL_REWARD_POOL_PERCENT_OF_PROFIT`**, 직업별 분배 **`POWERBALL_REWARD_WEIGHT_*`** 로 정합니다. 당일 **체크한 직업만** 가중치를 다시 합산해 같은 비율을 유지합니다.  
        - 네 직업 직업당 **12:7:3**, 군주 **7:3:2** (1~3위). 수혜자는 직업별 레벨 랭킹(`characters.gm` 제외)입니다.
        ...
        **아데나 지급 (접속 불필요)**  
        ...
        - **접속 중**인 캐릭터는 메모리 인벤이 우선하므로, 반드시 `gm_adena_delivery`(첫 아데나 스택 수량)를 넣어 서버가 즉시 반영합니다. (자동 정산·재지급 경로는 이와 동일하게 동작합니다.)
        """
    )
...
if _pb_ti == 0:
    ...
    with c3:
        ...
        if st.button(
            "🔄 DB 새로고침",
            key="pb_manual_refresh_tab1",
            help="연결 ping 후 일일 손익을 DB에서 즉시 다시 집계합니다 (자동 갱신 대기 없이).",
        ):
            st.rerun()
    ...
    with st.expander("⚙️ 포상 풀: 순이익 대비 % · 직업 가중치 (`config.py`)", expanded=False):
        st.caption(
            "서버 **순이익**에 대해 **몇 %를 랭커 포상 풀**로 쓸지와, 그 풀을 **직업별로 어떤 비율**로 나눌지 정합니다. "
            "아래 **정산 체크박스**에서 뺀 직업은 당일 몫에서 제외되고, **체크한 직업만** 가중치를 다시 합산해 비율을 유지합니다."
        )
        _cfg_pp = float(getattr(gm_config, "POWERBALL_REWARD_POOL_PERCENT_OF_PROFIT", 0.27)) * 100.0
        _col_pct, _col_sp = st.columns([1, 2])
        with _col_pct:
            _inp_pool_pct = st.number_input(
                "순이익 대비 포상 풀 (%)",
                min_value=0.0,
                max_value=100.0,
                value=float(min(100.0, max(0.0, _cfg_pp))),
                step=0.5,
                format="%.2f",
                key="pb_reward_cfg_pool_pct",
                help="예: 27 = 순이익의 27%를 총 포상 풀로. 나머지는 서버에 남는 금액으로 둡니다.",
            )
        with _col_sp:
            st.caption("직업 가중치는 **비율**만 의미합니다. (예: 기사 100·군주 50 → 군주가 기사의 절반 몫)")
        _g1, _g2, _g3, _g4, _g5 = st.columns(5)
        with _g1:
            _w_kn = st.number_input(
                "기사",
                min_value=0,
                max_value=99999,
                value=int(getattr(gm_config, "POWERBALL_REWARD_WEIGHT_KNIGHT", 100)),
                step=1,
                key="pb_reward_w_knight",
            )
        with _g2:
            _w_wz = st.number_input(
                "법사",
                min_value=0,
                max_value=99999,
                value=int(getattr(gm_config, "POWERBALL_REWARD_WEIGHT_WIZARD", 100)),
                step=1,
                key="pb_reward_w_wizard",
            )
        with _g3:
            _w_el = st.number_input(
                "요정",
                min_value=0,
                max_value=99999,
                value=int(getattr(gm_config, "POWERBALL_REWARD_WEIGHT_ELF", 100)),
                step=1,
                key="pb_reward_w_elf",
            )
        with _g4:
            _w_de = st.number_input(
                "다크",
                min_value=0,
                max_value=99999,
                value=int(getattr(gm_config, "POWERBALL_REWARD_WEIGHT_DARKELF", 100)),
                step=1,
                key="pb_reward_w_darkelf",
            )
        with _g5:
            _w_ry = st.number_input(
                "군주",
                min_value=0,
                max_value=99999,
                value=int(getattr(gm_config, "POWERBALL_REWARD_WEIGHT_ROYAL", 60)),
                step=1,
                key="pb_reward_w_royal",
            )
        if st.button("💾 포상 풀 설정 저장", type="primary", key="pb_save_reward_pool_cfg"):
            _ok_c, _msg_c = _update_gm_config_powerball_reward_pool(
                _inp_pool_pct, _w_kn, _w_wz, _w_el, _w_de, _w_ry
            )
            if _ok_c:
                queue_feedback("success", _msg_c)
                st.rerun()
            else:
                queue_feedback("error", _msg_c)

    d2 = st.date_input("정산일 (KST)", value=date.today() - timedelta(days=1), key="pb_reward_date")
    st.markdown("**정산에 포함할 클래스** (체크 해제 시 그 직업 몫은 **체크한 직업**에 **가중치 비율**로 재분배)")
    ...
    with r5:
        st.checkbox("군주", key="pb_chk_royal", help="끄면 군주 가중치 몫은 체크한 다른 직업에 재분배.")
