...
import pandas as pd
from utils.log_reader import get_manager_log_dir, list_log_dates, read_log_lines, parse_connect_line, clear_log_file
...
with st.sidebar:
    ...
    with st.expander("🗑️ 로그 초기화"):
        st.caption("선택한 날짜 로그 파일 내용을 비웁니다. 서버는 계속 기록합니다.")
        confirm_reset = st.checkbox("초기화 실행 확인", key="connect_confirm_reset")
        if st.button("로그 초기화", key="connect_reset", disabled=not confirm_reset):
