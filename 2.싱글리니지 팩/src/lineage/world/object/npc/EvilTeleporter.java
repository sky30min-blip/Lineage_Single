...
public class EvilTeleporter extends object {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		list.add(String.format("입장 레벨: %d이상 입장 가능", GmEventSettings.getMinLevel(GmEventSettings.DEVIL, Lineage.devil_level)));
		...
		{
			int dp = GmEventSettings.getPlayTimeSeconds(GmEventSettings.DEVIL, Lineage.devil_play_time);
			list.add(String.format("진행 시간: %s", dp < 60 ? dp + "초" : (dp / 60) + "분"));
		}
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
		if (action.equalsIgnoreCase("devil_teleport")) {
			if (pc.getGm() > 0 || DevilController.isOpen) {
				if (pc.getGm() > 0 || (GmEventSettings.getMinLevel(GmEventSettings.DEVIL, Lineage.devil_level) <= pc.getLevel())) {
					...
					ChattingController.toChatting(pc, String.format("악마왕의 영토는 %d레벨 이상 입장 가능합니다.", GmEventSettings.getMinLevel(GmEventSettings.DEVIL, Lineage.devil_level)), Lineage.CHATTING_MODE_MESSAGE);
				}
			} else {
