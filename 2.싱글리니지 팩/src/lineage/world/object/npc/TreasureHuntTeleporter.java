...
public class TreasureHuntTeleporter extends object {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		list.add(String.format("입장 레벨: %d이상 입장 가능", GmEventSettings.getMinLevel(GmEventSettings.TREASURE, Lineage.Treasuress_level)));
		list.add(String.format("입장 시간: %s", Lineage.Treasuress_dungeon_time));	
		{
			int ps = GmEventSettings.getPlayTimeSeconds(GmEventSettings.TREASURE, Lineage.Treasuress_play_time);
			list.add(String.format("진행 시간: %s", ps < 60 ? ps + "초" : (ps / 60) + "분"));
		}
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
		if (action.equalsIgnoreCase("tbox_teleport")) {
			if (pc.getGm() > 0 || TreasureHuntController.isOpen) {
				if (pc.getGm() > 0 || (GmEventSettings.getMinLevel(GmEventSettings.TREASURE, Lineage.Treasuress_level) <= pc.getLevel())) {
				
						pc.toPotal(Util.random(32801, 32804), Util.random(32803, 32805), 807);
			
				} else {
					ChattingController.toChatting(pc, String.format("보물찾기는 %d레벨 이상 입장 가능합니다.", GmEventSettings.getMinLevel(GmEventSettings.TREASURE, Lineage.Treasuress_level)), Lineage.CHATTING_MODE_MESSAGE);
				}
			} else {
