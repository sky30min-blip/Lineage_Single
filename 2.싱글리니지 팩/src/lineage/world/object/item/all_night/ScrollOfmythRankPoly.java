	...
	public void toClick(Character cha, ClientBasePacket cbp) {
		if (!cha.isDead() && !cha.isWorldDelete() && cha.getInventory() != null && cha instanceof PcInstance) {
			...
			if (!World.isTeamBattleMap(cha) && !cha.isFishing()) {
				...
				case 1:
					if (cha.getClassSex() == 0)
						polyName = "남자기사 랭커 야도란";
					else
						polyName = "여자기사 랭커 야도란";
					break;
				case 2:
					if (cha.getClassSex() == 0)
						polyName = "남자요정 랭커 야도란";
					else
						polyName = "여자요정 랭커 야도란";
					break;
				case 3:
					if (cha.getClassSex() == 0)
						polyName = "남자법사 랭커 야도란";
					else
						polyName = "여자법사 랭커 야도란";
					break;
				case 4:
					if (cha.getClassSex() == 0)
						polyName = "남다엘 랭커";
				...
				if (p != null) {
					...
					// 아이템 수량 갱신
					if (!getItem().getName().contains("신화 랭킹 변신 카드")) {
