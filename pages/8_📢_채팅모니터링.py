...
채팅 영역만 fragment로 주기 갱신해 깜빡임 없이 표시.
"""
...
# 채팅 영역만 이 간격으로 갱신 (전체 페이지 리런 없음 → 깜빡임 없음, 원본 툴처럼 부드럽게)
run_every_interval = None if not auto_refresh else timedelta(seconds=CHAT_POLL_SECONDS)

@st.fragment(run_every=run_every_interval)
def _chat_display_fragment():
    """채팅 로그만 주기적으로 다시 그리기. 나머지 화면은 그대로라 깜빡임 없음."""
...
# 채팅 로그 영역 — fragment로 이 부분만 2초마다 갱신 (화면 깜빡임 없음)
_chat_display_fragment()
...
with st.expander("❓ 채팅이 안 보이거나 전송이 안 될 때"):
    ...
    """)
st.caption("채팅 영역만 2초마다 갱신됩니다 (화면 깜빡임 없음)." if auto_refresh else "자동 새로고침이 꺼져 있습니다. 아래 버튼으로 수동 갱신.")
