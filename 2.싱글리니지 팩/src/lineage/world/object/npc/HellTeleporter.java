...
public class HellTeleporter extends object {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		list.add(String.format("입장 레벨: %d이상 입장 가능", GmEventSettings.getMinLevel(GmEventSettings.HELL, Lineage.hell_level)));
		...
		{
			int hp = GmEventSettings.getPlayTimeSeconds(GmEventSettings.HELL, Lineage.hell_play_time);
			list.add(String.format("진행 시간: %s", hp < 60 ? hp + "초" : (hp / 60) + "분"));
		}
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
		if (action.equalsIgnoreCase("hell_teleport")) {
			if (pc.getGm() > 0 || HellController.isOpen) {
				if (pc.getGm() > 0 || (GmEventSettings.getMinLevel(GmEventSettings.HELL, Lineage.hell_level) <= pc.getLevel())) {
					...
					ChattingController.toChatting(pc, String.format("지옥은 %d레벨 이상 입장 가능합니다.", GmEventSettings.getMinLevel(GmEventSettings.HELL, Lineage.hell_level)), Lineage.CHATTING_MODE_MESSAGE);
				}
			} else {
