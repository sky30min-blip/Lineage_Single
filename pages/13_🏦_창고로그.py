...
import pandas as pd
from utils.log_reader import get_manager_log_dir, list_log_dates, read_log_lines, clear_log_file
...
with st.sidebar:
    ...
    with st.expander("🗑️ 로그 초기화"):
        ...
        confirm_reset = st.checkbox("초기화 실행 확인", key="wh_confirm_reset")
        if st.button("로그 초기화", key="wh_reset", disabled=not confirm_reset):
            ok, msg = clear_log_file("창고 로그", date_str)
            if ok:
                st.success(msg)
            else:
                st.error(msg)
