...
public class WorldBossTeleporter extends object {
	...
	public void toTalk(PcInstance pc, ClientBasePacket cbp) {
		...
		list.add(String.format("입장 레벨: %d이상 입장 가능", GmEventSettings.getMinLevel(GmEventSettings.WORLDBOSS, Lineage.world_level)));
		...
		{
			int wp = GmEventSettings.getPlayTimeSeconds(GmEventSettings.WORLDBOSS, Lineage.world_play_time);
			list.add(String.format("진행 시간: %s", wp < 60 ? wp + "초" : (wp / 60) + "분"));
		}
	...
	public void toTalk(PcInstance pc, String action, String type, ClientBasePacket cbp) {
		if (action.equalsIgnoreCase("world_teleport")) {
			if (pc.getGm() > 0 || WorldBossController.isOpen) {
				if (pc.getGm() > 0 || (GmEventSettings.getMinLevel(GmEventSettings.WORLDBOSS, Lineage.world_level) <= pc.getLevel())) {
					...
					ChattingController.toChatting(pc, String.format("월드보스 토벌은 %d레벨 이상 입장 가능합니다.", GmEventSettings.getMinLevel(GmEventSettings.WORLDBOSS, Lineage.world_level)), Lineage.CHATTING_MODE_MESSAGE);
				}
			} else {
