...
with st.sidebar:
    ...
    with st.expander("🗑️ 로그 초기화"):
        ...
        confirm_reset = st.checkbox("초기화 실행 확인", key="enchant_confirm_reset")
        if st.button("로그 초기화", key="enchant_reset", disabled=not confirm_reset):
            ok, msg = clear_log_file("인첸트 로그", date_str)
            if ok:
                st.success(msg)
            else:
                st.error(msg)
